...
export default function App() {
    ...
    function setLongLat(long, lat, clickedOn) {
        map.current.flyTo({
            center: [lat, long],
            zoom: 8,
        })
                    ...
                    map.current.on("click", "unclustered-point", (e) => {
                        ...
                        const tsunami =
                            e.features[0].properties.tsunami === 1
                                ? "Yes"._()
                                : "No"._();
                        const title = e.features[0].properties.title.substring(
                            e.features[0].properties.title.indexOf("-") + 2
                        );
                        ...
                        const timeFormat =
                            USER.timeFormat === "24-Hour" ? "HH:mm" : "h:mm a";
                        const TIMESTAMP =
                            moment(e.features[0].properties.time)
                                .tz(USER.timeZone)
                                .format(`${USER.dateFormat} ${timeFormat}`) +
                            " " +
                            moment.tz(USER.timeZone).zoneAbbr();
